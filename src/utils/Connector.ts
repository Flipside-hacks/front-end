...
const useConnector = (chainID:number,networkName:string):(IConnector|null) =>{
    ...
    const connect:(IConnector|null) = (chainAddresses?.CROSSCHAIN_DAO && chainAddresses?.GOVERNANCE_TOKEN && networkName )?({
        chainID,
        network:networkName,
        adddress_campaign_factory: chainAddresses?.CAMPAIGN_FACTORY,
        adddress_crosschain_dao: chainAddresses?.CROSSCHAIN_DAO,
        adddress_campaign_satellite: chainAddresses?.CAMPAIGN_SATELLITE,
        adddress_governance_token: chainAddresses?.GOVERNANCE_TOKEN,
